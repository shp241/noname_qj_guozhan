			...
			use: {
				...
				ai: {
					order(skill, player) {
						...
						return max - Math.min(char1, char2) + 3 * Math.random() > 0 ? 1 : -1;
					},
					result: {
						player(player) {
							if (cardName == "diaohulishan") {
								return 0;
							}
...
const skill = {
			...
			1: {
				...
				async cost(event, trigger, player) {
					event.result = await player
						...
						.set("check", player.countCards("h") > 2 || player.countCards("j") > 0)
			...
			3: {
				...
				async cost(event, _trigger, player) {
					event.result = await player
						...
						.set("goon", player.needsToDiscard() || player.countCards("j") > 0)
						.set("ai", target => {
							var player = _status.event.player;
	...
	qj_guanxing: {
		...
		filter(event, player, name) {
			if (name == "phaseJieshuBegin") {
				return player.hasSkill("qj_guanxing_on");
			}
			return true;
		},
		async content(event, trigger, player) {
			...
			if ((!result.bool || !result.moved[0].length) && event.triggername == "phaseZhunbeiBegin") {
				player.addTempSkill("qj_guanxing_on");
			}
		},
		subSkill: {
			on: { charlotte: true },
			intro: { content: "回合结束时，你可以再发动一次“观星”。" },
		},
		ai: {
			guanxing: true,
		},
	...
	qj_longdan: {
		...
		group: ["qj_longdan_shamiss", "qj_longdan_draw", "qj_longdan_sha", "qj_longdan_shan", "qj_longdan_tao", "qj_longdan_jiu"],
		subSkill: {
			shamiss: {
				sub: true,
				audio: "longdan",
				...
				filter(event, player) {
					return event.skill == "qj_longdan_sha";
				},
				async cost(event, trigger, player) {
					event.result = await player
			...
			draw: {
				...
				filter(event, player) {
					...
					return event.skill == "qj_longdan_sha" || event.skill == "qj_longdan_shan";
				},
				async content(_event, _trigger, player) {
					player.draw();
	...
	qj_cangji: {
		...
		async content(event, trigger, player) {
			...
			if (cards.length == 1) {
				...
				let result = await player
					.chooseButton("藏机：请选择要防止弃置的牌", true, cards, 1)
					.set("ai", () => {
						if (ui.selected.buttons.length == 0) {
							return 1;
						}
	...
	qj_lieyong: {
		...
		async content(_event, trigger, player) {
			...
			let result = await player
				...
				.set("prompt", get.prompt("qj_lieyong", trigger.target))
				.setHiddenSkill("qj_lieyong")
				.set("ai", function () {
					var player = _status.event.player,
			...
			if (result.control != "cancel2") {
				...
				player.logSkill("qj_lieyong", target);
				if (result.index == 1) {
	...
	qj_niepan: {
		...
		async content(event, trigger, player) {
			player.awakenSkill("qj_niepan");
			player.storage.qj_niepan = true;
		...
		ai: {
			...
			skillTagFilter(player, tag, target) {
				if (player != target || player.storage.qj_niepan) {
					return false;
				}
			},
			save: true,
	...
	qj_kanpo: {
		...
		subSkill: {
			...
			view: {
				...
				intro: { content: "回合结束时，你可以观看当前回合角色的手牌。" },
			},
		},
	},
	qj_xiangle: {
	...
	qj_fanjian: {
		...
		mainSkill: true,
		init(player) {
			if (player.checkMainSkill("qj_fanjian")) {
				player.removeMaxHp();
			}
		},
		enable: "phaseUse",
		...
		async content(event, trigger, player) {
			...
			target.storage.qj_fanjian = cards[0];
			await player.give(cards[0], target);
			var suit = get.suit(target.storage.qj_fanjian);
			if (target.countCards("he", card => get.suit(card) == suit && lib.filter.cardDiscardable(card, target, "qj_fanjian")) < 2) {
				event.result = { control: "qj_fanjian_hp" };
			} else {
				event.result = await target
					.chooseControl("qj_fanjian_card", "qj_fanjian_hp")
					.set("choiceList", ["弃置所有" + get.translation(suit) + "牌（至少两张）", "失去1点体力"])
					.set("ai", async (event, player) => {
						var cards = player.getCards("he", {
							suit: get.suit(player.storage.qj_fanjian),
						});
						if (cards.length == 1) {
							return 0;
						}
			...
			if (event.result.control == "qj_fanjian_card") {
				await target.showHandcards();
				await target.discard(
					target.getCards("he", function (i) {
						return get.suit(i) == suit && lib.filter.cardDiscardable(i, target, "qj_fanjian");
					})
				);
				delete target.storage.qj_fanjian;
			} else {
				await target.loseHp();
			}
		},
	...
	qj_guose: {
		...
		filterTarget(card, player, target) {
			if (get.position(ui.selected.cards[0]) != "s" && lib.filter.cardDiscardable(ui.selected.cards[0], player, "qj_guose") && target.countCards("j") > 0) {
				return true;
			}
			if (player == target) {
				return false;
		...
		async content(event, trigger, player) {
			if (event.target.hasJudge("lebu")) {
				...
				await player.discardPlayerCard(event.target, "j", true);
			} else {
				await player.useCard({ name: "lebu" }, event.target, event.cards, false);
			}
			await player.draw();
		},
		ai: {
			...
			effect: {
				target(card, player, target) {
					if (get.type(card, "trick") == "trick" && !player.hasSkill("qj_qianxun_triggered")) {
						if (game.hasPlayer(p => p != player && lib.filter.targetEnabled2(card, player, p))) {
	...
	qj_guzheng: {
		...
		check(event, player, cards) {
			if (event.cards.length > 2 || get.attitude(player, event.player) > 0) {
				return true;
			}
			for (var i = 0; i < cards.length; i++) {
		...
		async cost(event, trigger, player) {
			...
			event.cards = cards;
			let str = "是否发动【固政】令" + get.translation(target) + "获得其中一张牌";
			if (cards.length > 1) {
				str += "，然后获得其余至多三张牌";
			}
			str += "？";
			const {
				...
				.set("check", lib.skill.qj_guzheng.check(trigger, player, event.cards))
				.set("ai", button => {
	...
	qj_jishi: {
		...
		async precontent(event, trigger, player) {
			if (_status.bannedGroups.includes("qun")) {
				await player.addTempSkill("qj_jishi_used");
			}
		},
		subSkill: {
			used: {
				...
				async content(event, trigger, player) {
					player.removeSkill("qj_jishi_used");
					let targets = game.players.filter(current => !player.isFriendOf(current) && current.group != "unknown" && !(current.isCharacterShibing(0) && current.isCharacterShibing(1)));
					let controlList = [];
					for (let target of targets) {
						if (!target.isCharacterShibing(0)) {
							controlList.push("主将");
						}
						if (!target.isCharacterShibing(1)) {
							controlList.push("副将");
						}
						controlList.push("cancel2");
						let result = await target
							.chooseControl("【济世】：是否用一个武将交换此武将？", controlList)
							.set("choice", () => {
								let char1 = get.guozhanRank(player.name1, target);
								let char2 = get.guozhanRank(player.name2, target);
								let huatuo = get.guozhanRank("qj_huatuo", target);
								if (target.isCharacterShibing(0) || char1 > char2) {
									return Math.random() > 0.3 * (char2 - huatuo) ? "主将" : "cancel2";
								}
								if (target.isCharacterShibing(1) || char2 >= char1) {
									return Math.random() > 0.3 * (char1 - huatuo) ? "副将" : "cancel2";
								}
								return "cancel2";
							})
							.forResult();
						if (result.control != "cancel2") {
							await player.swapCharacter(target, player.getGuozhanSkillZone("qj_jishi"), result.control == "主将" ? "main" : "vice");
							return;
						}
					}
				},
	...
	qj_xiaomeng: {
		...
		filter(event, player) {
			if (event.card.name != "juedou" || !event.card.isCard || player.hasSkill("qj_xiaomeng_used")) {
				return false;
			}
			if (event.targets) {
				if (
	...
	qj_xiongyi: {
		...
		limited: true,
		async contentBefore(event, trigger, player) {
			player.awakenSkill("qj_xiongyi");
		},
		async content(event, trigger, player) {
			let targets = game.filterPlayer(cur => cur.isFriendOf(player));
			targets.sortBySeat();
			for (let target of targets) {
				await target.draw(3);
			}
			if (player.isDamaged()) {
				...
				game.countPlayer(function (current) {
					if (current.group && current.group != "unknown") {
						groupCounts[current.group] = (groupCounts[current.group] || 0) + 1;
					}
				});
				let playerGroupCount = groupCounts[player.group] || 0;
				let minCount = Math.min(...Object.values(groupCounts));
				if (playerGroupCount == minCount) {
					player.recover();
				}
			}
		},
		...
		async content(event, trigger, player) {
			...
			if (result.bool) {
				if (game.hasPlayer(current => target != current && player.canUse("sha", current, false))) {
					...
					let result2 = await player
						.chooseTarget(str, str2, true, [1, 2], function (card, player, target) {
							return player.canUse("sha", target, false);
						})
						.set("filterOk", () => ui.selected.targets.includes(target))
						.set("ai", function (target) {
							var player = _status.event.player;
							return get.effect(target, { name: "sha" }, player, player);
						})
	...
	qj_xiaoni: {
		...
		multiline: false,
		async contentBefore(event, trigger, player) {
			player.awakenSkill("qj_xiaoni");
		},
		async content(event, trigger, player) {
			var card = { name: "sha", isCard: true };
			await player.useCard(card, event.targets, false).forResult();
			let targets = [];
			for (let target of event.targets) {
				if (
					target.getHistory("damage", function (evt) {
						return evt.skill == event.skill;
					}).length == 0 &&
			...
			if (targets.length) {
				let result = await player
					.chooseTarget("逍逆：请选择一名角色弃置其三张牌", true, (card, player, target) => targets.includes(target))
					.set("ai", target => -get.attitude(player, target))
					.forResult();
				if (result.bool) {
					await player.discardPlayerCard(result.targets[0], true, "he", 3);
				}
			}
		},
		ai: {
	...
	qj_chuitong: {
		...
		preHidden: true,
		async cost(event, trigger, player) {
			let list = trigger.player.getCards("e").filter(card => player.canEquip(card));
			if (list.length) {
				event.result = await player
					.choosePlayerCard(trigger.player, "e", get.prompt("qj_chuitong", trigger.player))
					.set("list", list)
		...
		async content(event, trigger, player) {
			player.logSkill("qj_chuitong", trigger.player);
			var card = event.links[0];
			await trigger.player.$give(card, player, false);
			await game.delay(0.5);
	...
	qj_xingtu: {
		...
		preHidden: true,
		filter(event, player, name) {
			return game.countPlayer2(current => current.getHistory("useCard", evt => get.is.yingbian(evt.card)).length > 0) > 0;
		},
		async cost(event, trigger, player) {
			let choiceList = [];
			if (player.countCards("hes", card => get.type(card) == "equip") > 0 && game.hasPlayer(target => player.canUse("tao", target, false))) {
				choiceList.push("将一张装备牌当【桃】对一名角色使用");
			}
			choiceList.push("摸一张牌");
			choiceList.push("cancel2");
			let result = await player
				.chooseControl(choiceList)
				.set("prompt", get.prompt("qj_xingtu", player))
				.set("choice", function () {
					if (
						game.countPlayer(function (current) {
					...
					if (
						...
						return Math.random() > 0.5 ? "将一张装备牌当【桃】对一名角色使用" : "摸一张牌";
					}
					return "摸一张牌";
				})
				.forResult();
		...
		async content(event, trigger, player) {
			if (event.cost_data == "将一张装备牌当【桃】对一名角色使用") {
				let result = await player
					.chooseCardTarget({
						position: "hes",
						filterCard(card, player) {
				...
				if (result.bool) {
					...
					if (targets.length) {
						player.useCard(cardx, cards, targets, false);
					}
				}
			} else {
				await player.draw();
	...
	qj_tairan: {
		...
		async content(event, trigger, player) {
			...
			let result = await player.chooseControl(list).set("prompt", "###你可以失去任意点体力###此回合结束时，你将恢复等量的体力").forResult();
			let num1 = 0,
				num2 = 0;
			if (result.control != "cancel2") {
				num1 = result.index;
			...
			if (
				...
				var result2 = await player
					.chooseToDiscard("he", [1, player.getDamagedHp()], "你可以弃置任意张牌", "此回合结束时，你将摸等量的牌。")
					.set("ai", function (card) {
						return 5.5 - get.value(card);
					})
					.forResult();
				if (result2.bool) {
					num2 = result2.cards.length;
				}
			}
			if (num1 > 0 || num2 > 0) {
				player.addTempSkill("qj_tairan_effect");
				if (player.getStorage("qj_tairan_effect").length == 0) {
					player.storage["qj_tairan_effect"] = [num1, num2];
				} else if (player.getStorage("qj_tairan_effect").length == 2) {
					player.storage.qj_tairan_effect[0] += num1;
					player.storage.qj_tairan_effect[1] += num2;
				}
			}
		},
		subSkill: {
			effect: {
				...
				filter(event, player) {
					var storage = player.getStorage("qj_tairan_effect");
				...
				intro: { content: "回合结束时，你回复#点体力，摸#张牌。" },
			},
		},
	},
	qj_yimie: {
			...
			if (trigger.cards.filterInD().length) {
				...
				if (trigger.card.isCard && trigger.cards.length == 1) {
					...
					if (player.canUse(card, trigger.player, false)) {
						...
						if (player.getHistory("sourceDamage", evt => evt.card == card).length) {
							return;
						}
					}
				}
	...
	qj_rouke: {
		...
		async content(event, trigger, player) {
			...
			player.addTempSkill("qj_rouke_link");
			player.markAuto("qj_rouke_link", trigger.card);
		},
		mod: {
			maxHandcardBase(player, num) {
		...
		subSkill: {
			...
			link: {
				...
				onremove: true,
				filter(event, player) {
					let cards = player.getStorage("qj_rouke_link");
					return (
						cards?.length &&
						!game.hasPlayer2(target => {
							return target.getHistory("damage", evt => evt.card && evt.card == cards[0]).length > 0;
						})
					);
				},
				async content(event, trigger, player) {
	...
	qj_zhouting: {
		...
		animationColor: "thunder",
		async contentBefore(event, trigger, player) {
			player.awakenSkill("qj_zhouting");
		},
		async content(event, trigger, player) {
			let i = 0;
			while (true) {
	...
	qj_shangshi: {
		...
		preHidden: true,
		filter(event, player) {
			return (event.player.countCards("h") > player.countCards("h") && player.countCards("hes") > 0 && player.canUse("sha", event.player, false)) || player.countCards("h") < player.getDamagedHp();
		},
		async cost(event, trigger, player) {
			let choiceList = [];
			if (trigger.player.countCards("h") > player.countCards("h") && player.countCards("hes") > 0 && player.canUse("sha", trigger.player, false)) {
				choiceList.push("将任意张牌当【杀】对当前回合角色使用");
			}
			if (player.countCards("h") < player.getDamagedHp()) {
				choiceList.push("摸牌至你已损失的体力值");
			}
			choiceList.push("cancel2");
			let result = await player
				.chooseControl(choiceList)
				.set("choice", function () {
					if (choiceList.includes("将任意张牌当【杀】对当前回合角色使用") && (get.attitude(player, trigger.player) > 0 || player.countCards("h") == 0)) {
						return "将任意张牌当【杀】对当前回合角色使用";
					}
					return choiceList.includes("摸牌至你已损失的体力值") ? "摸牌至你已损失的体力值" : "cancel2";
				})
				.forResult();
			event.result = {
				bool: result.control != "cancel2",
		...
		async content(event, trigger, player) {
			...
			if (control == "将任意张牌当【杀】对当前回合角色使用") {
				...
				if (result.bool) {
					...
					player.useCard(cardx, cards, target, false);
				}
			} else if (control == "摸牌至你已损失的体力值") {
	...
	qj_ejue: {
		...
		logTarget: "player",
		async cost(event, trigger, player) {
			let ownedSkills = player.getSkills(null, false, false).filter(skill => !lib.skill[skill].sub && !lib.skill[skill].charlotte);
			...
			if (player.hasMark("zhulianbihe_mark")) {
				ownedSkills.push("珠联璧合");
				choiceList.push("珠联璧合");
			}
			ownedSkills.push("cancel2");
			let result = await player
				...
				.set(
					...
					(() => {
						const uselessSkills = ownedSkills.filter(skill => {
							...
							if (player.awakenedSkills.includes(skill) && (info.limited || info.juexingji || info.dutySkill)) {
								return true;
							}
	...
	qj_qianqu: {
		...
		async cost(event, trigger, player) {
			...
			event.result = await player
				...
				.set("addCount", false)
				.forResult();
			player.markAuto("qj_qianqu", event);
		},
		async content(event, trigger, player) {
			let event1 = player.getStorage("qj_qianqu");
			if (!event1.length) {
				return;
			}
			var bool = game.hasPlayer2(function (current) {
				return (
					current.getHistory("damage", function (evt) {
						return evt.getParent("qj_qianqu_cost") == event1[0];
					}).length > 0
				);
			});
			if (bool) {
			...
			player.unmarkAuto("qj_qianqu", event1[0]);
		},
		subSkill: {
			yingbian: {
				charlotte: true,
				...
				async content(event, trigger, player) {
					...
					if (forced || goon) {
						...
						await game.yingbianEffect(trigger, () => {
							trigger.player.addTempSkill("qj_qianqu_discard");
						});
					}
				},
			},
		...
		discard: {
			...
			async content(event, trigger, player) {
				...
				player.removeSkill("qj_qianqu_discard");
			},
		},
	},
	qj_fuli: {
	...
	qj_fengwu: {
		...
		async content(event, trigger, player) {
			for (let target of event.targets) {
				await target.chooseToUse("【风物】：是否使用一张牌？", false, false).set("addCount", false);
			}
			let suits = [];
	...
	qj_mizhao: {
		...
		async content(event, trigger, player) {
			...
			if (result2.targets.length) {
				event.target2 = result2.targets[0];
	...
	qj_gounan: {
		...
		async content(event, trigger, player) {
			player.line(
				game.filterPlayer(current => current.isFriendOf(trigger.source) || current.isFriendOf(player)),
				"green"
			);
	...
	qj_dingzhu: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				.chooseTarget(get.prompt2("qj_dingzhu"), (card, player, target) => {
		...
		async content(event, trigger, player) {
			await event.targets[0].chooseUseTarget({ name: "juedou" }, true);
		},
	},
	qj_minsi: {
		audio: "minsi",
		...
		async content(event, trigger, player) {
			...
			if (event.bool) {
				...
				const bool = await trigger.player
					...
					.set("choice", get.effect(player, sha, trigger.player, trigger.player) > 0)
					.forResultBool();
				if (bool) {
					await trigger.player.useCard(sha, player, false);
				}
			}
		},
		subSkill: {
	...
	qj_qiuyuan: {
		...
		async content(event, trigger, player) {
			...
			event.player.addTempSkill("qj_qiuyuan_trigger");
			event.player.markAuto("qj_qiuyuan_trigger", [trigger.card]);
		},
		subSkill: {
			trigger: {
				trigger: {
	...
	qj_moukui: {
		...
		async cost(event, trigger, player) {
			...
			if (trigger.target.countDiscardableCards(player, "he") > 0) {
				list.push("选项二");
				list.push("执行两项");
				choiceList.push("弃置" + get.translation(trigger.target) + "的一张牌");
				choiceList.push("依次执行以上两项。然后若此【杀】未对其造成过伤害，则其弃置你的一张牌。");
			}
			list.push("cancel2");
			const result = await player
				.chooseControl(list)
				.set("choiceList", choiceList)
				.set("ai", function () {
					if (list.includes("执行两项") && Math.random() < 1 - trigger.target.countCards("h") * 0.2) {
						return "执行两项";
					}
					if (!list.includes("选项二") || trigger.target.isFriendOf(player) || Math.random() < 0.6) {
						return "选项一";
					}
					return "选项二";
				})
		...
		async content(event, trigger, player) {
			...
			if (control == "选项一" || control == "执行两项") {
				await player.draw();
			}
			if (control == "选项二" || control == "执行两项") {
				await player.discardPlayerCard(target, true, "he");
			}
			if (control == "执行两项") {
				player.addTempSkill("qj_moukui_2", "useCardAfter");
	...
	qj_chengzhao: {
		...
		ai: {
			unequip: true,
			skillTagFilter(player, tag, arg) {
				if (!arg || !arg.card || !arg.card.storage.qj_chengzhao) {
					return false;
				}
			},
		},
	...
	qj_aiwu: {
		trigger: { global: "damageEnd" },
		filter(event, player) {
			return event.num > 0 && event.player != player && event.player.isIn() && !player.hasSkill("qj_aiwu_used");
		},
		async cost(event, trigger, player) {
	...
	qj_juebie: {
		...
		filter(event, player) {
			return player.isFriendOf(event.player) && player != event.player;
		},
		preHidden: true,
		group: "qj_juebie_self",
		async cost(event, trigger, player) {
		...
		async content(event, trigger, player) {
			...
			if (result.control != "cancel2") {
				...
				if (result.control == "获得牌") {
					await target.gain(trigger.player.getCards("he"), trigger.player, "giveAuto", "bySelf");
				} else {
					await target.draw(3);
				}
			}
	...
	qj_jingnu: {
		...
		filter(event, player) {
			let evts = player.getHistory("useCard", evt => evt.getParent("phaseUse") == event);
			if (evts.length > 0 && evts.every(evt => evt.card.name == "sha")) {
				return true;
			}
			if (evts.length >= 2 && evts.slice(-2).every(evt => evt.card.name != "sha")) {
				return true;
			}
			return false;
		},
	...
	qj_weitun: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				.chooseTarget("威屯：视为对任意名角色使用【桃园结义】和【五谷丰登】",[1,Infinity])
				.set("ai", target => {
					return get.attitude(player, target);
				})
				.forResult();
	...
	qj_kannan: {
		...
		filter(event, player) {
			...
			return (
				game.hasPlayer(function (current) {
					...
					return current.group != "unknown" && player.canCompare(current);
		...
		filterTarget(card, player, target) {
			if (player.getStat()._qj_kannan) {
				for (let i of player.getStat()._qj_kannan) {
					if (target.isFriendOf(i)) {
						return false;
					}
				}
			}
			return current.group != "unknown" && player.canCompare(target);
		},
		async content(event, trigger, player) {
			const target = event.target;
			const result = await player.chooseToCompare(target).set("clear", false).forResult();
			let card1 = result.player;
			let card2 = result.target;
			if (result.num1 < result.num2) {
				let result1 = await target
					.chooseControl("获得牌", "cancel2")
					.set("choiceList", ["获得没赢的拼点牌并令" + get.translation(player) + "本回合使用下张【杀】伤害+1"])
					.set("choice", () => {
						return get.attitude(target, player) > 0 ? "获得牌" : "cancel2";
					})
					.forResult();
				if (result1.control != "cancel2" && get.position(card1) == "d") {
					await target.gain(card1, "gain2");
					if (!player.hasSkill("qj_kannan_damage")) {
				...
				let result1 = await player
					...
					.set("choice", "获得牌")
					.forResult();
				if (result1.control != "cancel2" && get.position(card2) == "d") {
		...
		async content(event, trigger, player) {
			if ((player == trigger.player && trigger.num1 <= trigger.num2) || (player != trigger.player && trigger.num2 <= trigger.num1)) {
				if (!player.hasSkill("qj_niju_add")) {
					player.addTempSkill("qj_niju_add");
				}
				if (!player.getStorage("qj_niju_add").length) {
					player.markAuto("qj_niju_add", 3);
				} else {
					player.storage["qj_niju_add"] += 3;
				}
			}
	...
	qj_quexiaojiang: {
		...
		filter(event, player) {
			...
			var targets1 = player.getHistory("useCard", e => e.card == evt.card)[0].targets;
			var targets2 = player.getHistory("useCard", e => e.card == event.card)[0].targets;
		...
		async cost(event, trigger, player) {
			var evt = player.getLastUsed(1);
			if (!evt || !evt.card || !player.getHistory("useCard", e => e.card == evt.card).length || !player.getHistory("useCard", e => e.card == trigger.card).length) {
				return false;
			}
			var targets1 = player.getHistory("useCard", e => e.card == evt.card)[0].targets;
			var targets2 = player.getHistory("useCard", e => e.card == trigger.card)[0].targets;
	...
	qj_wenhu: {
		...
		async content(event, trigger, player) {
			...
			for (let i = 0; i < event.targets.length; i++) {
				if (choices.length == event.targets.length) {
					choice.push(...choices);
					break;
				}
				const result = await player
			...
			await player.useCard({ name: "juedou", isCard: true }, event.targets);
		},
		ai: {
			order: 1,
			result: {
	...
	qj_jinfanjun: {
		...
		filter(event, player) {
			...
			if (player.storage.qj_jinfanjun && player.storage.qj_jinfanjun.length) {
				for (let c of event.cards2) {
					if (!player.storage.qj_jinfanjun.includes(get.color(c))) {
						return true;
					}
				}
				return false;
		...
		async content(event, trigger, player) {
			...
			for (let card of cards) {
				if (!colors.includes(get.color(card))) {
					colors.push(get.color(card));
				}
			}
			let colors1 = [];
			if (player.storage.qj_jinfanjun && player.storage.qj_jinfanjun.length) {
				for (let c of colors) {
					if (!player.storage.qj_jinfanjun.includes(c)) {
						colors1.push(c);
					}
				}
			} else {
	...
	qj_bingzhoulangqi_2: {
		...
		async content(event, trigger, player) {
			await event.target.useCard({ name: "juedou", isCard: true }, player, "noai");
			player.addTempSkill("qj_bingzhoulangqi_2_buff");
			player.markAuto("qj_bingzhoulangqi_2_buff", event.target);
		},
		ai: {
			order: 2,
			result: {
		...
		subSkill: {
			buff: {
				...
				mod: {
					globalFrom(from, to) {
						if (from.getStorage("qj_bingzhoulangqi_2_buff").includes(to)) {
							return -Infinity;
						}
					},
				},
				...
				sourceSkill: "qj_bingzhoulangqi_2",
			},
		},
	},
	qj_huangjinleishi: {
	...
	qj_huangjinleishi_2: {
		...
		filter(event, player) {
			if (
				!player.hasMark("qj_huangjinleishi_2_triggered") &&
				event.source.getHistory("lose", function (evt) {
					return (evt.relatedEvent || evt.getParent()) == event.getParent().getParent() && evt.hs && evt.hs.length == event.cards.length;
				}).length == 0
			) {
				player.markAuto("qj_huangjinleishi_2_triggered", "triggered");
				player.addTempSkill("qj_huangjinleishi_2_triggered");
				return player.countCards("h") > 0;
			}
			return false;
		},
	...
	qj_xiyuanjun_2: {
		...
		async content(event, trigger, player) {
			const cards = event.cards,
				target = trigger.player;
			target.showCards(cards, "展示手牌");
			player.addShownCards(cards, "visible_qj_xiyuanjun_2");
			target.addTempSkill("qj_xiyuanjun_2_gain");
		},
		subSkill: {
			gain: {
				group: "qj_xiyuanjun_2_3",
				onremove(player) {
					player.hideShownCards(player.getCards("h"), "visible_qj_xiyuanjun_2");
				},
				mod: {
					cardEnabled(card) {
						if (card.hasGaintag("visible_qj_xiyuanjun_2")) {
							return false;
						}
					},
				},
				...
				async content(event, trigger, player) {
					const hs = player.getCards("h", function (card) {
						return card.hasGaintag("visible_qj_xiyuanjun_2");
					});
					player.hideShownCards(player.getCards("h"), "visible_qj_xiyuanjun_2");
					if (hs.length && trigger.source != player) {
						await trigger.source.gain(hs, "gain2");
	...
	tl_simashi: {
		...createTonglingSkill("tl_simashi", "yc_fuxinsishi", "chuqibuyi", "♥【出其不意】").set("viewAs", { suit: "heart", tags: ["yingbian_zhuzhan", "yingbian_add"] }),
	},
	tl_jin_simayi: {
